...
interface MovieDao {
    ...
    @Query("select id, title, poster_path, overview, runtime, genre, language" +
            ", popularity, release_date, country from movie_table")
    fun listMovie(): LiveData<List<MovieDatabase>>

    @Query("select id, title, poster_path, overview, runtime, genre, language" +
            ", popularity, release_date, country from movie_table where id = :id")
